...
#include "backwardPhiCcDdtScheme.H"
...
template<class Type>
scalar backwardPhiCcDdtScheme<Type>::deltaT_() const
{
...
template<class Type>
scalar backwardPhiCcDdtScheme<Type>::deltaT0_() const
{
...
scalar backwardPhiCcDdtScheme<Type>::deltaT0_(const GeoField& vf) const
{
...
backwardPhiCcDdtScheme<Type>::fvcDdt
(
...
template<class Type>
tmp<fvMatrix<Type> >
backwardPhiCcDdtScheme<Type>::fvmDdt
(
...
template<class Type>
tmp<typename backwardPhiCcDdtScheme<Type>::fluxFieldType>
backwardPhiCcDdtScheme<Type>::fvcDdtPhiCorr
(
...
template<>
tmp<surfaceScalarField> backwardPhiCcDdtScheme<vector>::fvcDdtPhiCorr
(
    const volScalarField& rA,
...
)
{
    ...
    if (mesh().moving())
    {
        ...
        )
        {
            ...
            surfaceVectorField Unor0 = phi.oldTime()/mag(Sf.oldTime()) * nf0;
            surfaceVectorField Utang00 = fvc::interpolate(U.oldTime().oldTime()) - nf00*(fvc::interpolate(U.oldTime().oldTime()) & nf00);
            surfaceVectorField Unor00= phi.oldTime().oldTime()/mag(Sf.oldTime().oldTime()) * nf00;
            ...
            (
                ...
                (
                    ...
                    rDeltaT*ddtPhiCoeff
                   *(
                        coefft0*((UV0oV + fvc::interpolate(V0oV)*(Utang0 + Unor0)) & mesh().Sf())
                      - coefft00*((UV00oV + fvc::interpolate(V00oV)*(Utang00 + Unor00)) & mesh().Sf())
                    )/fvc::interpolate(1.0/rA)
                )
            );
...
template<class Type>
tmp<surfaceScalarField> backwardPhiCcDdtScheme<Type>::meshPhi
(
    const GeometricField<Type, fvPatchField, volMesh>& vf
)
{
