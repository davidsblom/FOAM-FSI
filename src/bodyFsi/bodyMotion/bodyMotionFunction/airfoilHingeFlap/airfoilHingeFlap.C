...
defineTypeNameAndDebug(airfoilHingeFlap, 0);
addToRunTimeSelectionTable(bodyMotionFunction, airfoilHingeFlap, dictionary);

// * * * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * * //
void airfoilHingeFlap::writeState(const scalar& time)
{
...
scalar airfoilHingeFlap::getFlapAngle(const scalar time) const
{
...
vector airfoilHingeFlap::getWingState(const scalar& time) const
{
	const scalar& pi = mathematicalConstant::pi;
...
tmp<Field<vectorField> > airfoilHingeFlap::calculatePosition(const scalar time)
{
    ...
    vector newFlapRotationOrigin = flapRotationOrigin_;
    newFlapRotationOrigin = (RzCurWing & ( flapRotationOrigin_ - rotationOrigin_ )) + rotationOrigin_;
    newFlapRotationOrigin += wingTranslationVector;

    //Set motion to all patches
    forAll(flapPatchIDs_,ipatch)//flapPatches are the first in patchIDs_ and thus in position list
    {
        vectorField rotationField = (RzCurFlap & ( position[ipatch] - newFlapRotationOrigin )) + newFlapRotationOrigin;
...
void airfoilHingeFlap::setWingPatchIDs(){
...
void airfoilHingeFlap::setPatchNamesAndIDs(){
...
tmp<Field<vectorField> > airfoilHingeFlap::getChildMotion()
{
...
void airfoilHingeFlap::setInitialPoints(){
    forAll(patchIDs_,ipatch){
        initialPoints_[ipatch] = mesh_.boundaryMesh()[patchIDs_[ipatch]].localPoints();
    }
}

const labelList airfoilHingeFlap::getChildMotionPatchIDs() const{
    return patchIDs_;
}

const wordList airfoilHingeFlap::getChildMotionPatchNames() const{
...
airfoilHingeFlap::airfoilHingeFlap(const dictionary& dict,const fvMesh& mesh, const word name):
...
flapRotationOrigin_(dict.lookup("flapRotationOrigin")),
flapPhaseShift_(0),
smoothStart_(false),
...
airfoilHingeFlap::~airfoilHingeFlap()
{
...
const Field<vectorField>& airfoilHingeFlap::getPosition() const
{
    return currentPosition_;
}

void airfoilHingeFlap::update()
{
