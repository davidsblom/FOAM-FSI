...
namespace rbf {
UnitCoarsening::UnitCoarsening(double tol,
    int minPoints,
    int maxPoints
    )
    :
...
void UnitCoarsening::compute(std::shared_ptr<RBFFunctionInterface> rbfFunction,
    ...
    for (int i = 0; i < maxPoints; i++) {
        ...
        *positionsInterpolationCoarse = *positions;
        ...
        bool convergence = locMax.value < tol && static_cast<int>(selectedPositions.size()) >= minPoints;
...
void UnitCoarsening::selectData(const std::unique_ptr<ElDistVector> & data,
