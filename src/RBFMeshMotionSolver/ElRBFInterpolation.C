...
ElRBFInterpolation::ElRBFInterpolation(std::shared_ptr<RBFFunctionInterface> rbfFunction,
...
void ElRBFInterpolation::compute(std::shared_ptr<RBFFunctionInterface> rbfFunction,
    std::unique_ptr<ElDistVector> positions,
    std::unique_ptr<ElDistVector> positionsInterpolation
    ) {
    assert(Phi->Height() == 0);
