...
ElRBFMeshMotionSolver::ElRBFMeshMotionSolver(const Foam::polyMesh & mesh,
    Foam::Istream &
    )
    :
    motionSolver(mesh),
    ...
    if (coarseningStrategy == "UnitCoarsening") {
        double tol = static_cast<double>(readScalar(subDict("UnitCoarsening").lookup("tol")));
    ...
    if (coarseningStrategy == "AdaptiveCoarsening") {
        double tol = static_cast<double>(readScalar(subDict("AdaptiveCoarsening").lookup("tol")));
        double reselectionTol = static_cast<double>(readScalar(subDict("AdaptiveCoarsening").lookup("reselectionTol")));
