...
void RBFInterpolation::evaluateH(const matrix & positions,
    matrix & H
    ) {
...
void RBFInterpolation::evaluatePhi(const matrix & positions,
...
void RBFInterpolation::compute(const matrix & positions,
    const matrix & positionsInterpolation
    ) {
    // Verify input
...
void RBFInterpolation::interpolate(const matrix & values,
 ...
 */
void RBFInterpolation::interpolate(const matrix & positions,
    const matrix & positionsInterpolation,
    const matrix & values,
    matrix & valuesInterpolation
    ) {
...
void RBFInterpolation::buildPhi(const matrix & positions,
    const matrix & positionsInterpolation
    ) {
    assert(positions.cols() == positionsInterpolation.cols());
    assert(positions.rows() > 0);
 ...
 */
void RBFInterpolation::interpolate2(const matrix & values,
    matrix & valuesInterpolation
    ) {
    assert(computed);
