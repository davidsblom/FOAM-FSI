...
namespace rbf {
AdaptiveCoarsening::AdaptiveCoarsening(double tol,
...
void AdaptiveCoarsening::compute(std::shared_ptr<RBFFunctionInterface> function,
...
void AdaptiveCoarsening::greedySelection(const std::unique_ptr<ElDistVector> & values,
    ...
    for (int i = 0; i < maxPoints; i++) {
        ...
        *positionsInterpolationCoarse = *positions;
        ...
        if (static_cast<int>(selectedPositions.size()) >= maxPoints)
            break;

        bool convergence = error < tol && static_cast<int>(selectedPositions.size()) >= minPoints;
...
std::unique_ptr<ElDistVector> AdaptiveCoarsening::interpolate(const std::unique_ptr<ElDistVector> & values) {
    ...
    if (largestError.second >= reselectionTol && !greedyPerformed) {
        ...
        if (static_cast<int>(selectedPositions.size()) < maxPoints / 2)
            clear = false;
...
void AdaptiveCoarsening::selectData(const std::unique_ptr<ElDistVector> & data,
    std::unique_ptr<ElDistVector> & selection
    ) {
    assert(selection->Height() == static_cast<int>(selectedPositions.size()));
