...
namespace fsi {
AndersonPostProcessing::AndersonPostProcessing(shared_ptr<MultiLevelFsiSolver> fsi,
    int maxIter,
...
void AndersonPostProcessing::fixedUnderRelaxation(vector & xk,
    vector & R,
    vector & yk
    ) {
    // Update solution x
...
void AndersonPostProcessing::performPostProcessing(const vector & x0,
    vector & xk
    ) {
    vector y(x0.rows());
    y.setZero();
...
void AndersonPostProcessing::performPostProcessing(const vector & y,
    const vector & x0,
    vector & xk
    ) {
