...
namespace tubeflow {
TubeFlowLinearizedSolidSolver::TubeFlowLinearizedSolidSolver(int N,
    scalar nu,
    scalar rho,
    scalar h,
    scalar L,
...
void TubeFlowLinearizedSolidSolver::solve(const fsi::matrix & input,
    fsi::matrix & output
    ) {
    assert(input.rows() == N);
    assert(input.cols() == 1);
...
void TubeFlowLinearizedSolidSolver::solve(const fsi::vector & p,
    fsi::vector & a
    ) {
    assert(init);
    assert(rStages.size() == uStages.size());
