...
SDCFluidSolver::SDCFluidSolver(const std::string & name,
...
void SDCFluidSolver::setSolution(const fsi::vector & solution,
    const fsi::vector & f
    ) {
    p = pStages.at(0);
...
void SDCFluidSolver::evaluateFunction(const int,
    const fsi::vector &,
    const scalar,
    fsi::vector & f
    ) {
...
void SDCFluidSolver::implicitSolve(bool corrector,
    const int k,
    const int,
    const scalar t,
    const scalar dt,
...
void SDCFluidSolver::getVariablesInfo(std::deque<int> & dof,
...
void SDCFluidSolver::prepareImplicitSolve(bool,
    const int,
    const int,
    const scalar,
    const scalar,
