...
namespace tubeflow {
TubeFlowSolidSolver::TubeFlowSolidSolver(scalar a0,
    scalar cmk,
    scalar p0,
    scalar rho,
    scalar L,
...
void TubeFlowSolidSolver::solve(const fsi::matrix & input,
    fsi::matrix & output
    ) {
    assert(input.rows() == N);
    assert(input.cols() == 1);
...
void TubeFlowSolidSolver::solve(const fsi::vector & p,
    fsi::vector & a
    ) {
