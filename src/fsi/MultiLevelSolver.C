...
namespace fsi {
MultiLevelSolver::MultiLevelSolver(shared_ptr<BaseMultiLevelSolver> solver,
...
void MultiLevelSolver::interpToCouplingMesh(matrix & data,
...
void MultiLevelSolver::interpToMesh(matrix & data,
    matrix & dataInterpolated
    ) {
    assert(data.cols() > 0);
    assert(data.rows() > 0);
...
void MultiLevelSolver::solve(matrix & input,
