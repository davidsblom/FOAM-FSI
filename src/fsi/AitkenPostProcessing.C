...
namespace fsi {
AitkenPostProcessing::AitkenPostProcessing(shared_ptr<MultiLevelFsiSolver> fsi,
...
void AitkenPostProcessing::performPostProcessing(const vector & x0,
    vector & xk
    ) {
    vector y(x0.rows());
    y.setZero();
...
void AitkenPostProcessing::performPostProcessing(const vector & y,
    const vector & x0,
    vector & xk
    ) {
