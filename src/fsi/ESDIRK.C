...
namespace sdc {
ESDIRK::ESDIRK(std::shared_ptr<SDCSolver> solver,
...
void ESDIRK::getSourceTerm(const bool /*corrector*/,
    const int k,
    const int /*sweep*/,
    const scalar deltaT,
    fsi::vector & rhs,
...
void ESDIRK::setFunction(const int k,
    const fsi::vector & f,
...
void ESDIRK::setOldSolution(int timeIndex,
    const fsi::vector & result
    ) {
