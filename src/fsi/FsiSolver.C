...
namespace fsi {
FsiSolver::FsiSolver(shared_ptr<BaseMultiLevelSolver> fluid,
 ...
 * x = S ( p ), p = F ( x ).
 */
void FsiSolver::evaluate(const vector & input,
    vector & output,
    vector & R
    ) {
    assert(fluid->init);
...
void FsiSolver::extrapolateData() {
    ...
    } else if (secondOrderExtrapolation) {
...
bool FsiSolver::isConvergence(const vector &,
    const vector &
    ) {
    assert(false);
    return true;
