...
defineTypeNameAndDebug(kkLOmega2008, 0);
addToRunTimeSelectionTable(RASModel, kkLOmega2008, dictionary);

// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

tmp<volScalarField> kkLOmega2008::fv(const volScalarField& Ret) const
{
    return(1.0 - exp(-sqrt(Ret)/Av_));
}


tmp<volScalarField> kkLOmega2008::fINT() const
{
    return
    (
        min
        (
            kl_/(Cint_*(kl_ + kt_ + k0_)),
...
tmp<volScalarField> kkLOmega2008::fSS(const volScalarField& omega) const
{
    return(exp(-sqr(Css_*nu()*omega/(kt_ + k0_))));
}


tmp<volScalarField> kkLOmega2008::Cmu(const volScalarField& S) const
{
    return(1.0/(A0_ + As_*(S/(omega_ + omega0_))));
}


tmp<volScalarField> kkLOmega2008::BetaTS(const volScalarField& Rew) const
{
    return(scalar(1) - exp(-sqr(max(Rew - CtsCrit_, scalar(0)))/Ats_));
}


tmp<volScalarField> kkLOmega2008::fTaul
(
...
tmp<volScalarField> kkLOmega2008::alphaT
(
    const volScalarField& lambdaEff,
...
tmp<volScalarField> kkLOmega2008::fOmega
(
    const volScalarField& lambdaEff,
...
tmp<volScalarField> kkLOmega2008::gammaBP(const volScalarField& omega) const
{
    return
    (
        min
...
tmp<volScalarField> kkLOmega2008::gammaNAT
(
...
tmp<volSymmTensorField> kkLOmega2008::R() const
{
...
tmp<volSymmTensorField> kkLOmega2008::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> kkLOmega2008::divDevReff(volVectorField& U) const
{
    return
    (
...
tmp<fvVectorMatrix> kkLOmega2008::divDevRhoReff
(
