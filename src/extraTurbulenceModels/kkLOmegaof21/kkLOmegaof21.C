...
defineTypeNameAndDebug(kkLOmegaof21, 0);
addToRunTimeSelectionTable(RASModel, kkLOmegaof21, dictionary);

// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

tmp<volScalarField> kkLOmegaof21::fv(const volScalarField& Ret) const
{
    return(1.0 - exp(-sqrt(Ret)/Av_));
}


tmp<volScalarField> kkLOmegaof21::fINT() const
{
    return
    (
        min
...
tmp<volScalarField> kkLOmegaof21::fSS(const volScalarField& omega) const
{
    return(exp(-sqr(Css_*nu()*omega/(kt_ + k0_))));
}


tmp<volScalarField> kkLOmegaof21::Cmu(const volScalarField& S) const
{
    return(1.0/(A0_ + As_*(S/(omega_ + omega0_))));
}


tmp<volScalarField> kkLOmegaof21::BetaTS(const volScalarField& Rew) const
{
    return(scalar(1) - exp(-sqr(max(Rew - CtsCrit_, scalar(0)))/Ats_));
}


tmp<volScalarField> kkLOmegaof21::fTaul
(
...
tmp<volScalarField> kkLOmegaof21::alphaT
(
    const volScalarField& lambdaEff,
...
tmp<volScalarField> kkLOmegaof21::fOmega
(
    const volScalarField& lambdaEff,
...
tmp<volScalarField> kkLOmegaof21::gammaBP(const volScalarField& omega) const
{
    return
    (
        min
...
tmp<volScalarField> kkLOmegaof21::gammaNAT
(
...
tmp<volSymmTensorField> kkLOmegaof21::R() const
{
...
tmp<volSymmTensorField> kkLOmegaof21::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> kkLOmegaof21::divDevReff(volVectorField& U) const
{
    return
    (
