...
TubeFlowExplicitLinearSolidSolver::TubeFlowExplicitLinearSolidSolver(int N,
    scalar nu,
    scalar rho,
    scalar h,
    scalar L,
...
void TubeFlowExplicitLinearSolidSolver::solve(const matrix & input,
    matrix & output
    ) {
    assert(input.rows() == N);
    assert(input.cols() == 1);
...
void TubeFlowExplicitLinearSolidSolver::solve(const fsi::vector & p,
    fsi::vector & a
    ) {
...
fsi::vector TubeFlowExplicitLinearSolidSolver::evaluateFunction(const fsi::vector & rn,
    const fsi::vector & pn,
    const fsi::vector & un
    ) {
    assert(rn.rows() == N);
