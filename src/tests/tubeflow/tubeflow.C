  ...
  )
{
  ...
  #pragma omp parallel for collapse(6), schedule(dynamic,1)
          ...
          for ( int iOrder = 0; iOrder < 3; iOrder++ )
          {
            for ( unsigned i = 0; i < fsiSolvers.size(); i++ )
            {
              ...
              if ( fsiSolver == "RPM" || fsiSolver == "MM" || fsiSolver == "OSM" || fsiSolver == "ASM" || fsiSolver == "ML-IQN-ILS" || fsiSolver == "ASM-ILS" )
              {
                ...
                shared_ptr<MultiLevelSolver> fluidSolver( new MultiLevelSolver( fineModelFluid, fineModelFluid, rbfInterpToCouplingMesh, rbfInterpToMesh, 0, nbLevels - 1 ) );
                ...
                shared_ptr<MultiLevelSolver> solidSolver( new MultiLevelSolver( fineModelSolid, fineModelSolid, rbfInterpToCouplingMesh, rbfInterpToMesh, 1, nbLevels - 1 ) );
                ...
                if ( fsiSolver == "RPM" || fsiSolver == "MM" || fsiSolver == "OSM" || fsiSolver == "ASM" || fsiSolver == "ASM-ILS" )
                {
                  for ( int level = 0; level < nbLevels - 1; level++ )
                  {
                    ...
                    if ( fsiSolver == "OSM" )
                      spaceMapping = shared_ptr<SpaceMapping> ( new OutputSpaceMapping( fineModel, coarseModel, maxIter, nbReuse, reuseInformationStartingFromTimeIndex, singularityLimit, iOrder ) );
