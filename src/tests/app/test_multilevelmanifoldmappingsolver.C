...
class MultiLevelManifoldMappingSolverParametrizedTest : public TestWithParam< std::tr1::tuple<bool, int, int, int, int, bool> >
{
protected:
  ...
  virtual void SetUp()
  {
    ...
    multiLevelFluidSolver = shared_ptr<MultiLevelSolver> ( new MultiLevelSolver( fluid, fineModelFluid, 0, 1 ) );
    multiLevelSolidSolver = shared_ptr<MultiLevelSolver> ( new MultiLevelSolver( solid, fineModelSolid, 1, 1 ) );
    ...
    solvers->push_back( manifoldMappingSolver );
    models->push_back( coarseModel );
    models->push_back( fineModel );

    shared_ptr<SpaceMappingSolver> coarseModelSolver = manifoldMappingSolver;
    ...
    multiLevelFluidSolver = shared_ptr<MultiLevelSolver> ( new MultiLevelSolver( fineModelFluid, fineModelFluid, 0, 2 ) );
    multiLevelSolidSolver = shared_ptr<MultiLevelSolver> ( new MultiLevelSolver( fineModelSolid, fineModelSolid, 1, 2 ) );
    ...
    manifoldMapping = shared_ptr<ManifoldMapping> ( new ManifoldMapping( fineModel, coarseModelSolver, maxIter, singularityLimit, nbReuse, reuseInformationStartingFromTimeIndex ) );
    ...
    solver = new MultiLevelSpaceMappingSolver( solvers, models );
...
INSTANTIATE_TEST_CASE_P( testParameters, MultiLevelManifoldMappingSolverParametrizedTest, ::testing::Combine( Bool(), Values( 0 ), Values( 0 ), Values( 3 ), Values( 20, 40 ), Bool() ) );

TEST_P( MultiLevelManifoldMappingSolverParametrizedTest, object )
{
  ASSERT_TRUE( true );
}

TEST_P( MultiLevelManifoldMappingSolverParametrizedTest, timeStep )
{
