...
TEST_F( ASMILSSolverTest, solveTimeStep )
{
  ...
  ASSERT_EQ( solver->fineModel->fsi->nbIter, 14 );
  ASSERT_EQ( fineModelFluid->nbRes, 70 );
  ASSERT_EQ( fineModelFluid->nbJac, 42 );

  solver->solveTimeStep();

  ASSERT_EQ( solver->fineModel->fsi->nbIter, 25 );
  ASSERT_EQ( fineModelFluid->nbRes, 125 );
  ASSERT_EQ( fineModelFluid->nbJac, 75 );
  ...
  ASSERT_EQ( solver->fineModel->fsi->nbIter, 64 );
  ASSERT_EQ( fineModelFluid->nbRes, 320 );
  ASSERT_EQ( fineModelFluid->nbJac, 192 );
}
