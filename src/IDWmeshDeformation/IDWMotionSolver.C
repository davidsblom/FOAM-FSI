...
namespace Foam
{
  defineTypeNameAndDebug( IDWMotionSolver, 0 );
  ...
  void IDWMotionSolver::makeControlIDs()
  {
  ...
  void IDWMotionSolver::moveMesh( pointField & newPoints ) const
  {
    ...
    fsi::matrix positions( controlIDs_.size(), 2 );
    fsi::matrix positionsInterpolation( internalIDs_.size(), 2 );
    fsi::matrix values( controlIDs_.size(), 2 );
    fsi::matrix valuesInterpolation( internalIDs_.size(), 2 );
    positions.setZero();
    positionsInterpolation.setZero();
    values.setZero();
    ...
    forAll( controlIDs_, i )
    {
      for ( int j = 0; j < positions.cols(); j++ )
        positions( i, j ) = mesh().points()[controlIDs_[i]] [j];
    }

    forAll( internalIDs_, i )
    {
      for ( int j = 0; j < positionsInterpolation.cols(); j++ )
        positionsInterpolation( i, j ) = mesh().points()[internalIDs_[i]] [j];
    }

    forAll( controlIDs_, i )
    {
      for ( int j = 0; j < values.cols(); j++ )
        values( i, j ) = newPoints[controlIDs_[i]][j];
    }

    interpolation_->interpolate( positions, positionsInterpolation, values, valuesInterpolation );
    ...
    forAll( internalIDs_, i )
    {
      ...
      {
        newPoints[internalIDs_[i]][j] = valuesInterpolation( i, j );
      }
    }
  }

  void IDWMotionSolver::setAndOrderPatchIDs()
  {
  ...
  void IDWMotionSolver::setMovingControlPointsIDs()
  {
  ...
  const labelList IDWMotionSolver::movingControlPointsIndices() const
  {
  ...
  void IDWMotionSolver::gatherControlPointsExt()
  {
  ...
  void IDWMotionSolver::prepareParallelRun()
  {
    if ( Pstream::parRun() )
    {
  ...
  void IDWMotionSolver::checkInterpolationDimension()
  {
    ...
    if ( twoDPointCorrectorRBF_.required() )
    {
      if ( dim == "ThreeD" )
      {
        FatalErrorIn( "void IDWMotionSolver::setInterpolationDimension" )
  ...
  IDWMotionSolver::IDWMotionSolver(
    const polyMesh & mesh,
    Istream &
    )
    :
    ...
    interpolation_( new fsi::RBFInterpolationCoarsening( 1.0e-3 ) )
  {
  ...
  void IDWMotionSolver::setMotion( const Field<vectorField> & m )
  {
    if ( m.size() != mesh().boundaryMesh().size() )
    {
      FatalErrorIn( "void IDWMotionSolver::setMotion(const Field<vectorField>& m)" )
  ...
  void IDWMotionSolver::setMotion( const vectorField & m )
  {
    if ( m.size() != movingIDs_.size() )
    {
      FatalErrorIn( "void IDWMotionSolver::setMotion(const vectorField& m)" )
  ...
  const vectorField & IDWMotionSolver::movingPoints() const
  {
    return movingPoints_;
  }

  tmp<pointField> IDWMotionSolver::curPoints() const
  {
  ...
  void IDWMotionSolver::solve()
  {
    // Motion must be set by using setMotion();
  }

  void IDWMotionSolver::updateMesh( const mapPolyMesh & )
  {
